...
class RestoreTrashImagesJob implements ShouldQueue
{
    ...
    {
        // Move original image from trash if exist.
        if (isset($this->file['details']['original'])) {
            $orgNewPath = str_replace($this->config['trash_start_path'] . '/', '', $this->file['details']['original']['path']);
        ...
        if (isset($this->file['details']['resize'])) {
            foreach ($this->file['details']['resize'] as $height => $val) {
                $newPath = str_replace($this->config['trash_start_path'] . '/', '', $val['path']);
